...
def coupling_eq(eqs,functions,delta_t,dx,dy,target):
    couples = []
    ...
    def discretization(eq,u):
        ...
        eq = eq.subs({
            ...
            u.forward.dx:(-u.forward.shift(x, -x.spacing) + u.forward)/dx,
            u.forward.dy:(-u.forward.shift(y, -y.spacing) + u.forward)/dy
            })
        return eq
    ...
    for i,eq in enumerate(eqs):
        ...
        pde = pde.subs(goal.dt,(goal.forward - goal)/delta_t)
        print(pde)
        pde = solve(pde,goal)
