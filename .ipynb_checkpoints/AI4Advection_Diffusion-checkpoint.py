...
import math
from filter.filter_generator import filter_generator
import numpy as np
import sympy as sp
...
class AI4Advection_Difussion(nn.Module):
    def __init__(self,u,dx,dy,dt,cx,cy,nx,ny,Dx, Dy):
        super(AI4Advection_Difussion, self).__init__()
        self.dx = dx
        self.dy = dy
        self.dt = dt
        self.cx = cx
...
class PDE_Solver(nn.Module):
    def __init__(self,eq,u,dx,dy,dt,nx,ny):
        super(PDE_Solver, self).__init__()
        A = filter_generator(eq,u,dt,dx,dy)
